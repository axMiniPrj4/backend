...
class ErdRoom:
    project_id: int
    peers: dict[str, ErdPeer] = field(default_factory=dict)
    lock: asyncio.Lock = field(default_factory=asyncio.Lock)


class ErdHub:
    def __init__(self) -> None:
        self._rooms: dict[int, ErdRoom] = {}
        self._global_lock = asyncio.Lock()

    async def _room(self, project_id: int) -> ErdRoom:
        async with self._global_lock:
            room = self._rooms.get(project_id)
            if room is None:
                room = ErdRoom(project_id=project_id)
                self._rooms[project_id] = room
            return room

    async def join(self, project_id: int, peer: ErdPeer) -> None:
